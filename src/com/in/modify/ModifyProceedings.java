...
/**
 * Servlet implementation class ModifyProceedings
 */
@WebServlet("/ModifyProceedings")
public class ModifyProceedings extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ModifyProceedings() {
        super();
		...
		try {
			System.out.println("docid:"+copyno[i]+"pdate:"+position[i]+"Branch"+branch);
			  if (modify[i].trim().equals("yes") && !copyno[i].trim().isEmpty()){
				  ...
				  String query1 = "update proceedings set clocation ='"+clocation[i]+"',ceditor='"+ceditor[i]+"' where docid='"+docid[i]+"';";
